...
def _shape(protection, codeowners, permission, me, branch):
    ...
    team_membership_unknown = (not listed and
                               any(name.startswith("@") for name in landers or []))
    ...
    return {
        ...
        "can_land": (True if listed or admin_pass else
                     None if team_membership_unknown else False),
    ...
    if gate["landers"] is not None:
        ...
        if gate.get("as_admin"):
            out.append("push su %s riservati a %s: tu ci arrivi come admin "
                       ...
                       % (gate["branch"], who))
        elif gate["can_land"] is None:
            out.append("la restrizione include un team: va verificata la tua "
                       "appartenenza prima di decidere chi può mergiare su %s"
                       % gate["branch"])
