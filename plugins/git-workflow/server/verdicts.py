...
def _landing(gate):
    ...
    """
    if not gate or gate.get("can_land", True) is True:
        return None
    if gate.get("can_land") is None:
        return ("verifica l'appartenenza al team autorizzato su %s" % gate["branch"],
                "decision", "asks")
...
def handoff(row, repo, merge_command):
    ...
    return {"kind": "prompt", "label": "Copia prompt per l'agente",
            "text": "Usa pr-loop solo per la PR #%s di %s: %s. Contesto dal desk: %s."
                    % (n, repo, todo, context)}
...
def issue_handoff(row, repo):
    if row["assignees"]:
        return None
    return {"kind": "prompt", "label": "Copia prompt per l'agente",
            "text": "Usa issue-triage e nella selezione prendi la #%s di %s (%s, %s): "
